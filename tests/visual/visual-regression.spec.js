...
test.describe('Visual Regression', () => {
  ...
  test.describe('Authenticated pages', () => {
    test.beforeEach(async ({ page }) => {
      ...
      await expect(page).toHaveURL(/\/(dashboard|setup|acceso)?$/i, { timeout: 15000 });
    });

    test('dashboard visual', async ({ page }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(3000); // Wait for charts/animations
      await expect(page).toHaveScreenshot('dashboard.png', {
        fullPage: true,
        maxDiffPixelRatio: 0.15, // More tolerance for dynamic data (stats, charts, timestamps)
      });
    });
